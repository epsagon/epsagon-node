...
do
    ...
    LAYER_VERSION=$(aws lambda publish-layer-version --layer-name epsagon-node-layer --description "Epsagon Node.js layer that includes pre-installed packages to get up and running with monitoring and distributed tracing" --content S3Bucket=epsagon-layers-${region},S3Key=epsagon-node-layer.zip --compatible-runtimes nodejs12.x nodejs10.x nodejs8.10 nodejs6.10 --license-info MIT --region ${region} | jq '.Version')
    sleep 3
